...
# functions
smoothing_function <- function(seurat, expMat, genes, path, z_cutoff = 3, windowsize=150){
...
gg_female <- readRDS("seurat_Gg_female_path2.rds") #path2 trajectory extracted
hs_female <- readRDS("seurat_Hs_female.rds")


hs_female <- ScaleData(hs_female, features = rownames(hs_female))

hs_female$Path2 <- rep(1, ncol(hs_female))
names(hs_female$Path2) <- colnames(hs_female)
gg_female$Path2 <- rep(1, ncol(gg_female))
names(gg_female$Path2) <- colnames(gg_female)

genes4use <- intersect(VariableFeatures(gg_female), VariableFeatures(hs_female))


# run
pseudotime <- gg_female$pseudotime
names(pseudotime) <- colnames(gg_female)

p_gg <- make_dynamic_heatmap(seurat = gg_female, expMat = gg_female@assays$RNA@scale.data[genes4use, ],
                            genes = genes4use, paths = c("Path2"), pseudotime = pseudotime, windowsize = 50, repel = F)



pseudotime <- hs_female$pseudotime
names(pseudotime) <- colnames(hs_female)

p_hs <- make_dynamic_heatmap(seurat = hs_female, expMat = hs_female@assays$RNA@scale.data[genes4use, ],
...
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == 2] <- "A"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == 5] <- "B"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == 4] <- "C"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == 3] <- "D"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == 1] <- "E"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == "A"] <- "F1"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == "B"] <- "F2"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == "C"] <- "F3"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == "D"] <- "F4"
gc_df$gene_clusters_gg[gc_df$gene_clusters_gg == "E"] <- "F5"

gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == 5] <- "a"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == 2] <- "b"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == 4] <- "c"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == 1] <- "d"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == 3] <- "e"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == "a"] <- "f1"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == "b"] <- "f2"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == "c"] <- "f3"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == "d"] <- "f4"
gc_df$gene_clusters_hs[gc_df$gene_clusters_hs == "e"] <- "f5"

gc_df <- gc_df[order(gc_df$gene_clusters_hs), ]
gc_df <- gc_df[order(gc_df$gene_clusters_gg), ]

# write.csv(gc_df, "Human_Chicken_GeneClusterInfo_Female.csv")