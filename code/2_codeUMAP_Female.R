...
seurat <- seurat[, seurat$sex == "Female"]

sce_female <- as.SingleCellExperiment(seurat)
sce_female <- sce_female[rowSums(sce_female@assays@data$counts) != 0,]
...
saveRDS(sce_female, "sce_female_merged_QC_normalized.rds")
...
length(top.hvgs) #750 genes

seurat <- as.Seurat(sce_female)
...
# saveRDS(seurat, "seurat_female_QC.rds")
...
ortho_hum4chic <- ortho[ortho$chicken_symbol %in% rownames(seurat), ] #Ortholog of human genes against chicken, for seurat features
...
# saveRDS(seurat, "seurat_female_QC_CCcorrected.rds")
